  ...
  if [[ ${EXIT_CODE} -ne 0 ]]; then
    ...
    for TASK in "$@"; do
      ...
      analyze_1)
        ...
        ;;
      command_00)
        echo 'dart test -p chrome,vm -c dart2js,exe -r expanded --suite-load-timeout=60s'
        dart test -p chrome,vm -c dart2js,exe -r expanded --suite-load-timeout=60s || EXIT_CODE=$?
        ;;
      command_01)
