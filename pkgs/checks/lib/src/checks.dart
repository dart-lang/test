...
final class _ReplayContext<T> implements Context<T> {
  ...
  FutureOr<Iterable<String>> describe({required bool allowAsync}) {
    ...
    if (result is Future) {
      return result.then((_) => context.detail(context, null).expected.skip(1));
    }
  ...
  void expect(
    ...
    _interactions.add((c) {
      c.expect(clause, predicate, predicateNoun: predicateNoun);
    });
  }
  ...
  Future<void> expectAsync(
    ...
    _interactions.add(
      (c) => c.expectAsync(clause, predicate, predicateNoun: predicateNoun),
    );
  ...
  void expectUnawaited(
    ...
    _interactions.add((c) {
      c.expectUnawaited(clause, predicate, predicateNoun: predicateNoun);
    });
  }
  ...
  Subject<R> nest<R>(
    ...
    _interactions.add((c) {
      final subject = c.nest(
        label,
        extract,
        atSameLevel: atSameLevel,
        addPredicate: addPredicate,
      );
  ...
  Future<Subject<R>> nestAsync<R>(
    ...
    _interactions.add((c) async {
      final nested = await c.nestAsync(
        label,
        extract,
        null,
        addPredicate: addPredicate,
