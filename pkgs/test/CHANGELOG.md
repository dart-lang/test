...
- Update to the latest `test_api` and `test_core`.
...
- When `addTearDown()` is called within a call to `setUpAll()`, it runs its
  callback after _all_ tests instead of running it after the `setUpAll()`
  callback.
...
- The expanded reporter will now _always_ print the full, untruncated test name.
