...
void main() {
  ...
  test("running a load test causes LoadSuite.suite to emit a suite", () async {
    ...
    expect(suite.entries, hasLength(1));

    expect(suite.suite, completion(equals(innerSuite)));
    var liveTest = await suite.entries.single.load(suite);
    await liveTest.run();
  ...
  group("changeSuite()", () {
    test("returns a new load suite with the same properties", () {
      ...
      expect(newSuite.entries.single.name, equals(suite.entries.single.name));
    });
