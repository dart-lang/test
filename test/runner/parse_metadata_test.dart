...
String _sandbox;
String _path;
AnalysisSessionManager _analysisSessionManager;

void main() {
  setUp(() {
    _sandbox = createTempDir();
    _path = p.join(_sandbox, "test.dart");
    _analysisSessionManager = new AnalysisSessionManager();
  });
  ...
  parse(String path) {
    return parseMetadata(path, new Set(), _analysisSessionManager);
  }

  test("returns empty metadata for an empty file", () {
    new File(_path).writeAsStringSync("");
    var metadata = parse(_path);
  ...
  group("@TestOn:", () {
    ...
    group("throws an error for", () {
      test("a named constructor", () {
        ...
        expect(() => parse(_path), throwsFormatException);
      });
